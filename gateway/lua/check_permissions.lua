...
local public_routes = {
    ...
    ["/v1/auth/logout"] = true,
    ["/v1/financial/ping"] = true,
    ["/v1/financial/payment/webhook"] = true,
    ["/v1/financial/payment/callback"] = true
}
...
if public_routes[uri] then
    ...
    if uri:match("^/v1/financial/") then
        ngx.var.upstream = "http://financial-service"
    else
        ngx.var.upstream = "http://fleet-service"
    end
...
local res, err = httpc:request_uri("http://127.0.0.1:8001/api/validate-token-front", {
    method = "GET",
